...
u64 gltf_getBufferViewsSize(zj_Value* bufferViews_json, GLTFModel* model){
    ...
    while(currentBufferView = zj_ArrayGet(bufferViews_json, bufferViewsCount)){
        bufferViewsCount++;

        zj_Value* name = zj_ObjGet(currentBufferView, "name");
        if(name != 0) bufferLengthAccumulator += strlen(zj_GetStr(name)) + 1;

        bufferLengthAccumulator += sizeof(GLTFBufferView);
    }
...
byte* gltf_fillBufferViewsBuffer(zj_Value* bufferViews_json, GLTFModel* model, byte* bufferPointer){
    ...
    for(u32 i = 0; i < model->bufferViewsCount; i++){
        currentBufferView = zj_ArrayGet(bufferViews_json, i);
