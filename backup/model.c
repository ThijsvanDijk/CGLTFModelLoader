...
unsigned char engine_model_make(const char* path, Model* store){
    ...
    char total_path[MAX_LEN];
    strncpy(total_path, cwd, MAX_LEN);
    strncat(total_path, path, MAX_LEN - strlen(cwd));
...
void engine_model_draw(Model* model, unsigned int shader){
    for(unsigned int i = 0; i < model->meshes_count; i++){
        engine_mesh_draw(model->meshes + i * sizeof(Mesh), shader);
    }
}

unsigned char engine_model_load(const struct aiScene* scene, Model* store){

    // store->meshes = (Mesh*) malloc(store->meshes_count * sizeof(Mesh));

    // engine_model_process_node(scene->mRootNode, scene, store);

    // aiReleaseImport(scene);
    return 1;
}

void engine_model_process_node(const struct aiNode* node, const struct aiScene* scene, Model* store){
...
void engine_model_process_mesh(const struct aiMesh* mesh, const struct aiScene* scene, Mesh* store_mesh, const char* dir){
    // /*
    //     LOAD VERTICES
    // */    
...
void engine_model_load_material(const struct aiMaterial* mat, enum aiTextureType type, TEXTURE_TYPE custom_type, Mesh* store_mesh, const char* dir, unsigned int* loaded_tex){
...
unsigned int engine_texture_from_file(const char* path, const char* directory){
